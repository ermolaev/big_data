...
basic_auth_parameters = Base64.encode64("#{node['ambari']['admin_user']}:#{node['ambari']['admin_password']}")


http_request 'Init Blueprints' do
  url      "http://#{ambari_server_fqdn}:8080/api/v1/blueprints/#{blueprint_name}"
  action   :post
...
http_request 'Init Cluster' do
  url      "http://#{ambari_server_fqdn}:8080/api/v1/clusters/#{cluster_name}"
  action   :post
  headers  ({ "AUTHORIZATION" => "Basic #{basic_auth_parameters}", "X-Requested-By" => "ambari-cookbook" })
